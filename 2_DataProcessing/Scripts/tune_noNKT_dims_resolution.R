# 基于“无 NKT”对象的 dims × resolution 调参与最终降维/聚类/UMAP
...
#   - 最终对象与图：2_DataProcessing/2_Doublet_Removed/RDS/nk.integrated.noNKT.tuned.rds
...
# 1) 配置
# ---------------------------
rds_path <- "2_DataProcessing/2_Doublet_Removed/RDS/nk.integrated.noNKT.rds"
...
# 选择标签列
label_col <- if ("singleR.pruned" %in% colnames(nk@meta.data)) "singleR.pruned" else if ("singleR.label" %in% colnames(nk@meta.data)) "singleR.label" else NULL
...
ensure_pca <- function(obj, target_dims = max(dims_grid)) {
  ...
  # 选择用于降维的 assay：优先 SCT 有可变基因，否则 RNA
  ...
  if (dr_assay == "RNA") {
    ...
    message("PCA 保障：使用 SCT 路线（VariableFeatures/Scale/RunPCA）")
    feats <- VariableFeatures(obj)
  }
...
# 写出指标表
metrics_csv <- file.path(tune_data, "nk_noNKT_tuning_metrics.csv")
...
if (nrow(df_metrics) > 0) {
  ...
  p_hm_sil <- ggplot(df_plot, aes(x = res_lab, y = dims, fill = median_silhouette)) +
    ...
    labs(title = "Median Silhouette across dims x res (noNKT)",
         x = "resolution", y = "dims") +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
  ggsave(file.path(tune_plots, "heatmap_median_silhouette_dims_vs_res_noNKT.png"), p_hm_sil, width = 10, height = 6, dpi = dpi_high)
  ggsave(file.path(tune_plots, "heatmap_median_silhouette_dims_vs_res_noNKT.pdf"), p_hm_sil, width = 10, height = 6, device = "pdf")

  p_hm_nc <- ggplot(df_plot, aes(x = res_lab, y = dims, fill = n_clusters)) +
    geom_tile(color = "white") +
    scale_fill_viridis_c(option = "B", na.value = "#f0f0f0") +
    labs(title = "Number of Clusters across dims x res (noNKT)",
         x = "resolution", y = "dims") +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
  ggsave(file.path(tune_plots, "heatmap_n_clusters_dims_vs_res_noNKT.png"), p_hm_nc, width = 10, height = 6, dpi = dpi_high)
  ggsave(file.path(tune_plots, "heatmap_n_clusters_dims_vs_res_noNKT.pdf"), p_hm_nc, width = 10, height = 6, device = "pdf")
}
...
best_per_dims_csv <- file.path(tune_data, "nk_noNKT_tuning_best_per_dims.csv")
utils::write.csv(cands$best_per_dims, best_per_dims_csv, row.names = FALSE)
best_global_csv <- file.path(tune_data, "nk_noNKT_tuning_top_candidates.csv")
...
nk <- FindClusters(nk, graph.name = graph_nm, resolution = res_i, verbose = FALSE)
  ...
  if ("timepoint" %in% colnames(nk@meta.data)) {
    p_split <- DimPlot(
      ...
      ncol = 2
        ...
        ggtitle(sprintf("UMAP tuning (noNKT) | dims=%d, res=%.1f", dims_i, res_i))

    fname_core <- sprintf("UMAP_noNKT_tuning_dims%d_res%s_byTimepoint", dims_i, safe_res(res_i))
  ...
  if (!is.null(label_col) && label_col %in% colnames(nk@meta.data)) {
    p_lab <- DimPlot(
      ...
      group.by = label_col,
      label = FALSE
    ) + ggtitle(sprintf("UMAP tuning (noNKT) | dims=%d, res=%.1f | colored by %s", dims_i, res_i, label_col))
    fname_lab <- sprintf("UMAP_noNKT_tuning_dims%d_res%s_bySingleR", dims_i, safe_res(res_i))
...
# 保存最终参数
sel_file <- file.path(tune_logs, "selected_params.txt")
writeLines(c(
...
# 最终对象保存
out_rds_final <- "2_DataProcessing/2_Doublet_Removed/RDS/nk.integrated.noNKT.tuned.rds"
...
if ("timepoint" %in% colnames(nk@meta.data)) {
  p_final_tp <- DimPlot(
    ...
    ncol = 2
      ...
      ggtitle(sprintf("UMAP (noNKT final) | dims=%d, res=%.1f | by timepoint", dims_best, res_best))
  ggsave(file.path(tune_plots, "UMAP_noNKT_final_byTimepoint.png"), p_final_tp, width = 12, height = 10, dpi = dpi_high)
}
...
if (!is.null(label_col) && label_col %in% colnames(nk@meta.data)) {
  p_final_lab <- DimPlot(
    ...
    group.by = label_col,
    label = FALSE
  ) + ggtitle(sprintf("UMAP (noNKT final) | dims=%d, res=%.1f | colored by %s", dims_best, res_best, label_col))
  ggsave(file.path(tune_plots, "UMAP_noNKT_final_bySingleR.png"), p_final_lab, width = 10, height = 8, dpi = dpi_high)
}
...
message("noNKT 调参与最终降维流程完成。所有产物已写入：", tune_base)
